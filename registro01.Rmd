...
#   shinyjs::enable("button8vos")
# }else{
#   shinyjs::disable("button8vos")
#   }
#   })
library(shinyjs)
v <- reactiveValues(OK = c(F,F))
observeEvent(input$buttonA, {
    v$OK[1] <- lista2A()$marcaBoton
  })

observeEvent(input$buttonB, {
    v$OK[2] <- lista2B()$marcaBoton
  })
...
listaW49<-eventReactive(input$button8vos,{
  ...
  fluidRow(
        ...
        # ,
        # hidden(
          actionButton("button8vos","Aceptar")
          # )
      # )
  #,width = '1090px')
        ),
...
column(7,
renderTable({
  if(listaW49()=="MENSAJE SIN CONCLUIR PRIMERA FASE"){
    m<-matrix("PRIMERA FASE SIN CONCLUIR CORRECTAMENTE")
    colnames(m)<-"¡¡¡O.O!!!!"
    m
  }else if(!is.null(listaW49()$data_EquipoPartido)){
        if(lista2A()$marcaBoton & lista2B()$marcaBoton){
          if(listaW49()$marcaBoton){
            if(sum(listaW49()$data_EquipoPartido$Empate==0)){
              as.matrix(listaW49()$data_EquipoPartido[1:4,c("Grupo","Partido","Equipo")])
            }else{
...
# observe({
#   toggleState("button8vos", condition = input$buttonA)
#   })
