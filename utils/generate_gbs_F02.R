generate_gbs_F02<-function(alldocs=alldocs,resultados_reales=resultados_reales,matches=c(49,50,51,52,53,54,55,56)){#,
  ...
  alldocs_selecF02_NF<-alldocs%>%
    ...
    filter(Grupo=="W" & Ganado==1 & Codigo2!="Final") # El problema de filtrar por Ganado es que 
                                  # no se podr�an saber los pron�sticos 
  ...
  gb_nomb_codigo2<-alldocs_selecF02_pterminados%>%
    ...
    summarise(Ganadores_Fase_acertados=as.integer(sum(I_Ganados)),#/2),
    ...
    ungroup()%>%
    mutate(
              Puntos_primlugar=if_else(Codigo2=="Final",8*Ganadores_Fase_acertados,0),
              Puntos_seglugar=if_else(Codigo2=="Final",4*Resultado_correcto_partido_Perdedor,0),
              Puntos_terlugar=if_else(Codigo2=="Tercer",2*Ganadores_Fase_acertados,0),
              ...
              Puntos_Partido = 5*Ganadores_Fase_acertados + Puntos_primlugar+Puntos_seglugar+Puntos_terlugar
              )
  
  gb_nomb<-gb_nomb_codigo2%>%
    # ungroup()%>%
    group_by(nomconcursante,folio)%>%
    summarise(Ganadores_TotalesF02_acertados=sum(Ganadores_Fase_acertados),
