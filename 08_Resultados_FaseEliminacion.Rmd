...
```{r}
sm2<-streamingmatch_F02(alldocs = alldocs,resultados_reales = resultados_reales)
```
...
datatable(sm2$sm%>%select(-nomconcursante,
                          -Resultado_correcto_partido_Perdedor,
                          -Puntos_primlugar,
                          -Puntos_seglugar,
                          -Puntos_terlugar)%>%rename(Puntos_por_ganadores=Puntos_Partido)%>%rename(Fase=Codigo2),
...
names(tb1)<-c("Concursante","folio","Ganadores Acertados","Pts x 1er Lugar","Pts x 2do Lugar","Pts x 3er Lugar","Pts Totales")
# datatable(tb1,
#           extensions = 'FixedColumns',
#           options=list(
#             dom = 'RMDCT<"clear">lfrtip',
